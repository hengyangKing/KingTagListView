...
+(instancetype)defaultConfig {
    ...
    //基类
    config.titleFont([UIFont fontWithName:@"PingFangSC-Regular"size:14]).radius(6.f);
    //
    config.buttonFont([UIFont fontWithName:@"PingFangSC-Regular"size:12]).borderWidth(1.f).tintColor([UIColor redColor]).needHiddenButton(NO).normalTitle(@"全选").selectedTitle(@"取消全选");
    return config;
}
#pragma mark set
